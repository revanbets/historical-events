...
(function () {
  ...
  // Load captureEnabled state from storage asynchronously
  let captureEnabledPromise = new Promise((resolve) => {
    chrome.storage.local.get('captureEnabled', (result) => {
      resolve(result.captureEnabled !== undefined ? result.captureEnabled : true);
    });
  });

  let captureEnabled = true; // cached value for performance

  // Initialize captureEnabled from storage
  captureEnabledPromise.then(value => {
    captureEnabled = value;
  });

  // Listen for toggle state changes from popup
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.type === 'CAPTURE_TOGGLE_CHANGED') {
  ...
  document.addEventListener('mouseup', async (e) => {
    ...
    // Check if capture is enabled before showing bubble
    // Wait for the initial state to load if needed
    const isEnabled = await captureEnabledPromise.then(() => captureEnabled);
    if (!isEnabled) {
      hideBubble();
      return;
    }

...
async function loadState() {
  try {
    ...
    captureEnabled = stored.captureEnabled !== undefined ? stored.captureEnabled : true;
  } catch (e) {
...
function updateCaptureToggleUI() {
  const btn = $('capture-toggle-btn');
  if (!btn) return;
  if (captureEnabled) {
    btn.textContent = '⏸';
    btn.title = "Toggle 'Save to DB' popup (currently: ON) — click to disable";
    btn.classList.remove('disabled');
  } else {
    btn.textContent = '▶';
    btn.title = "Toggle 'Save to DB' popup (currently: OFF) — click to enable";
...
function bindEvents() {
  ...
  // Capture toggle
  $('capture-toggle-btn').addEventListener('click', handleCaptureToggle);
...
async function handleCaptureToggle() {
  ...
  updateCaptureToggleUI();
  // Notify all tabs that capture state has changed
  const tabs = await chrome.tabs.query({});
  tabs.forEach(tab => {
    chrome.tabs.sendMessage(tab.id, { type: 'CAPTURE_TOGGLE_CHANGED', enabled: captureEnabled }).catch(() => {});
  });
}
